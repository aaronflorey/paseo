...
Crush is also a direct provider. Paseo starts one managed, loopback-only `crush server` process per configured Crush runtime and uses its HTTP/SSE v1 API; it does not use Crush's ACP mode or attach to the default Crush TUI server. Startup probes `/v1/health`, `/v1/version`, and the required route surface; a missing route is reported as an actionable Crush upgrade error. One server hosts multiple workspaces, and Crush deduplicates workspaces by resolved cwd. Sessions in the same cwd therefore share the selected large/small models, permission-skip mode, launch environment, MCP state, and other workspace configuration. Model changes made in Paseo are persisted to that project's `.crush/crush.json`; permission mode is runtime state and is reapplied from Paseo's stored agent config after a server restart.

Each Paseo Crush session owns a distinct client UUID and live workspace SSE claim. The claim must be opened immediately after workspace creation and released only after any ephemeral native session is deleted. Crush may exit automatically after the final claim disappears, so the process manager treats exit as a normal generation boundary and starts a new server on the next acquisition. A workspace data-directory lock means another standalone Crush instance owns that project; report the lock and ask the user to close the other instance rather than attempting to attach to its server.

Crush's server API has no per-session system-prompt field. Paseo does not rewrite Crush provider configuration or prepend hidden text to user messages: `systemPrompt` and `daemonAppendSystemPrompt` are ignored with a visible Paseo timeline notice. Configure instructions through Crush's native CRUSH.md/AGENTS.md and provider prompt settings. Crush's configured MCP servers continue to work, but Paseo MCP injection and native Paseo tools are disabled because those settings would be shared across every session in the workspace.
...
OpenCode uses one Paseo-managed `opencode serve` process for every active OpenCode agent. The OpenCode SDK scopes project operations with its directory header/query, so sessions from different projects can safely share that process. Do not reintroduce per-agent helper processes: every helper writes the same OpenCode SQLite database, and concurrent servers can contend on that database. Forced catalog refresh rotates the shared helper only when it has no active session references; otherwise it reuses the current generation.

OpenCode caches configuration in directory-scoped project instances inside that shared process. Every project-scoped operation must therefore hold a lease keyed by helper generation and normalized directory. When the final same-directory lease closes, serialize `instance.dispose({ directory })` before releasing the shared-server reference; never dispose while another session or metadata request for that directory is active. A failed disposal stays dirty and must be retried before the next acquisition, with an explicit refresh error if the retry also fails. Other directories on the helper remain active and untouched. This means project config edits take effect after the final session or request in that directory closes, while active sessions retain the config they loaded.

The shared process cannot carry per-agent environment variables. Paseo injects a small OpenCode plugin through `OPENCODE_CONFIG_CONTENT`; the plugin resolves environment variables by OpenCode session ID from an authenticated loopback context bridge before shell execution, then removes the bridge URL and token from the spawned shell environment. Parent-owned OpenCode child sessions inherit their parent's context, while an adopted child gets a direct binding for its own Paseo agent and falls back to the inherited binding if that agent closes. The token prevents accidental cross-session reads over loopback; it is not a security boundary against other same-UID processes or user-installed OpenCode plugins, which can inspect the helper process environment.

OpenCode MCP injection is dynamic and project-scoped. Call OpenCode's `mcp.add` endpoint with the MCP server config and do not follow it with `mcp.connect`; `connect` only toggles MCP servers already present in OpenCode's own config. New OpenCode versions return `McpServerNotFoundError`/404 for `connect` after a dynamic add because the server is not config-backed, while older versions silently swallowed the same missing-config path. The internal `paseo` MCP URL is normalized so all agents in one project register the same server config. The injected OpenCode plugin adds the provider session ID to each internal tool call after model-side argument validation, and the daemon uses it to select the exact caller-scoped Paseo tool catalog. Keep that routing argument absent from the advertised tool schema.

Custom OpenCode MCP servers follow OpenCode's project scope, not Paseo's agent scope. Once one session adds a custom server name, other sessions in the same directory can see that server. Paseo accepts repeated identical registration and rejects a different config under the same name with an explicit project-scope error; it must never silently treat a conflicting `already present` response as success. Different custom names accumulate for that OpenCode project until the shared helper rotates.
